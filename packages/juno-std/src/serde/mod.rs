use serde::{ de::{ self, value::{ I64Deserializer, U64Deserializer } }, Deserialize };
use std::{ fmt::{ Display, Formatter }, marker::PhantomData, str::FromStr };
...
impl<'de, T> de::Visitor<'de>
    for StringOrNumberVisitor<T>
    where T: Deserialize<'de>, T::Err: Display, T: FromStr
{
    type Value = T;
...
pub mod as_str {
    ...
    pub fn deserialize<'de, T, D>(deserializer: D) -> Result<T, D::Error>
        where T: FromStr, T::Err: Display, T: Deserialize<'de>, D: Deserializer<'de>
    {
...
pub mod option_as_str {
    ...
    pub fn deserialize<'de, T, D>(deserializer: D) -> Result<Option<T>, D::Error>
        where T: FromStr + Deserialize<'de>, T::Err: Display, D: Deserializer<'de>
    {
...
pub mod as_str_vec {
    ...
    pub fn serialize<S, T>(values: &[T], serializer: S) -> Result<S::Ok, S::Error>
        where S: Serializer, T: Display
    {
        let vec_of_strings: Vec<String> = values
            .iter()
...
pub mod as_base64_encoded_string {
    ...
    pub fn serialize<S>(values: &[u8], serializer: S) -> Result<S::Ok, S::Error> where S: Serializer {
        Binary::new(values.to_vec()).to_base64().serialize(serializer)
    }
}
